        ...
        <div class="info">
           {{-- <a itemprop="name" class="content-info__author"
            ...
            <div class="detail">
                ...
                {!! trans('index.postedon', ['time' => '<time  class="content-info__date">'.$post->published_at->format('d.m.Y H:i') .'</time>' ]) !!}
                @endif

                @if(get_buzzy_config('ShowUpdateDate') !== 'no' && @$post->haber_guncelleme > @$post->published_at)
                <em class="content-info__line">—</em> {!! trans('index.updatedon', ['time' => '<time class="content-info__date">'.@$post->haber_guncelleme->format('d.m.Y H:i') .'</time>' ]) !!}
                @endif
            </div>
        </div>
    </div>
