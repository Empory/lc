...
class PostEditorController extends Controller
{
    ...
    private function savePost($inputs, $post, $update = false)
    {
        try {
            ...
            if ($update) {
                ...
                if (makepreview($post->main_slide_image, 'b', 'posts') != $inputs['main_slide_image']) {
                    $main_slide_image = @$this->movePostImageMainSlide($inputs['main_slide_image'], $post_slug, $post->main_slide_image);

                    if (is_array($main_slide_image)) {
                        return $main_slide_image;
                    }

                    $post->main_slide_image = $main_slide_image;
                }

            } else {
                $post_thumb = $this->movePostImage($inputs['thumb'], $post_slug);
                $main_slide_image = @$this->movePostImageMainSlide($inputs['main_slide_image'], $post_slug);
            ...
            $post->haber_guncelleme = Carbon::now();
            $post->language = isset($inputs['language']) ? $inputs['language'] : get_default_language();

            $post->show_in_main_slide = @$inputs['show_in_main_slide'];
            $post->show_in_main_slide_2 = @$inputs['show_in_main_slide_2'];
            $post->show_in_breaking_news = @$inputs['show_in_breaking_news'];
            $post->show_in_home_videos = @$inputs['show_in_home_videos'];
    ...
    private function movePostImage($thumb, $slug, $post_thumb = null)
    {
        ...
        try {
            ...
            if ($thumb && null !== $thumb) {
                ...
                $image->name($slug . '_' . time() . '_' . rand(10000, 99999) );
                $image->setUrlFile($thumb);
                $image->make();
                $image->mime('jpg');
                $image->save(
                    [
                        'fit_width' => config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.preview-image_big_width', 365),
                        'fit_height' => config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.preview-image_big_height', 270),
                        'image_size' => 'b',
                    ]
                ); // move big image
                $image->save(
    ...
    private function movePostImageMainSlide($thumb, $slug, $post_thumb = null)
    {
        if (empty($slug)) {
            $slug = md5($slug . time());
        }
        ...
        try {
            ...
            if ($thumb && null !== $thumb) {
                ...
                $image->save(
                    [
                        'fit_width' => config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.slider-image_big_width', 1112),
                        'fit_height' => config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.slider-image_big_height', 448), //config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.preview-image_big_height', 440),
                        'image_size' => 'b',
                    ]
                ); // move big image
                $image->save(
                    [
                        'fit_width' => 310,//config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.preview-image_small_width', 400),
                        'fit_height' => 125, //config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.preview-image_small_height', 266),
                        'image_size' => 's',
                    ]
                );
    ...
    protected function getPostValidator($request, $post_id = null)
    {
        ...
        $v = $this->Postvalidator($request->only('title', 'slug', 'description', 'category', 'tags', 'pagination', 'type', 'thumb', 'main_slide_image', 'post_status', 'published_at', '_token','show_in_main_slide','show_in_main_slide_2','show_in_breaking_news','show_in_home_videos'), $post_id);
