    ...
    public function newUpload(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            try {
                ...
                } elseif ($type == 'preview_main_slide') {
                    $upload->make();
                    $upload->mime('jpg');
                    $upload->save([
                        'fit_width' => config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.slider-image_big_width', 1112),
                        'fit_height' => config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.slider-image_big_height', 448), //config('buzzytheme_' . get_buzzy_config('CurrentTheme') . '.preview-image_big_height', 250),
                    ]);
                } elseif ($type == 'answer') {
                    $upload->make();
                    $upload->mime('jpg');
