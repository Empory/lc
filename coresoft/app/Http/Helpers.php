...
function get_post_types($option = true)
{
    ...
    } else if(get_user_type() == 'Staff'){
        unset($post_types['list']);
        unset($post_types['quiz']);
        unset($post_types['poll']);
    } else {
        unset($post_types['opinion']);
        unset($post_types['list']);
        unset($post_types['poll']);
        unset($post_types['quiz']);
    }
    ...
    foreach ($post_types as $type => $value) {
        if (
            ...
            || get_buzzy_config('p_opinion') != 'on' && $type == 'opinion'
...
{
    if ($admin) {
        return route('admin.comments',  ['only' => 'unapprove']);
    }
